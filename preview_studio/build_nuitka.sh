...
RELEASE_NAME="TTS-Preview-Studio-Nuitka"
...
case "$(uname -s)" in
  Darwin)
    ...
    BIN_NAME="TTS Preview Studio"
    ;;
  MINGW*|MSYS*|CYGWIN*|Windows_NT)
    EXTRA+=(--windows-console-mode=disable)
    BIN_NAME="TTS Preview Studio.exe"
    ;;
  *)
    BIN_NAME="TTS Preview Studio"
    ;;
...
if [[ -f "$DIST_DIR/$BIN_NAME" ]]; then
  ...
  BIN_PATH="$(find "$DIST_DIR" -maxdepth 3 -type f \( -name "$BIN_NAME" -o -name 'PreviewStudio' -o -name 'TTS Preview Studio' \) ! -path '*.build/*' 2>/dev/null | head -1 || true)"
fi
...
TTS Preview Studio (Nuitka onefile)
==================================
...
3) Lần đầu trên máy: nếu TTS báo thiếu browser Camoufox:
     ...
     camoufox fetch
   (Camoufox browser tải riêng, không gói trong onefile.)
4) Chạy "TTS Preview Studio" (hoặc .exe trên Windows).
...
cat > "$RELEASE_DIR/run-preview-studio.bat" <<'EOF'
@echo off
cd /d "%~dp0"
if exist "TTS Preview Studio.exe" (
  start "" "TTS Preview Studio.exe"
) else if exist "TTS Preview Studio" (
  "TTS Preview Studio"
) else (
