...
__name(hasProxyCreds, "hasProxyCreds");
async function buildProxyPayload(row, poolRow) {
  const provider = poolRow && poolRow.provider || row.proxy_provider || "proxyxoay_net";
  const api_key = row.proxy_api_key || poolRow && poolRow.api_key || "";
  const host = row.proxy_host || poolRow && poolRow.host || "";
  const port = Number(row.proxy_port) || poolRow && Number(poolRow.port) || 0;
  const username = row.proxy_username || poolRow && poolRow.username || "";
  const password = row.proxy_password || poolRow && poolRow.password || "";
  const label = row.proxy_label || poolRow && poolRow.label || "";
...
__name(buildProxyPayload, "buildProxyPayload");
...
async function handleApi(req, env) {
  ...
  if (path === "/accounts" && method === "GET") {
    ...
    const rows = (r.results || []).map((a) => ({
      ...
      has_proxy: !!(a.proxy_host && a.proxy_username) || !!a.proxy_id || !!a.proxy_api_key,
